...
const BUCKETS = [
  // stone fruit
  ['peach',       ['복숭아','황도','백도','피치','살구','복숭아껍질','복숭아즙','복숭아잼','살구잼','넥타린','핵과','핵과류','잘 익은 과육','익은 과육','달콤한 과일','달콤한 과일 향미','apricot','peach','nectarine','white peach','stone fruit']],
  ['plum',        ['자두','자두잼','plum','damson','prune plum']],
  ['ume',         ['매실','매실청','매실향','청매','ume','greengage','green plum']],
  // citrus
  ['lemon',       ['레몬','시트러스','시트론','유자','라임','유자청','청귤','레몬필','레몬즙','라임즙','시트러스 산미','부드러운 감귤류 산미','감귤류 산미','감귤류','감귤 산미','lemon','citrus','citron','yuzu','lime','lemon zest','citric']],
  ['bergamot',    ['베르가못','베르가모트','얼그레이','얼그레이 티','홍차 같은 느낌','얼그레이 티 같은','bergamot','earl grey']],
  ...
  ['strawberry',  ['딸기','딸기잼','산딸기','베리류','잼처럼 진득한 단맛','잼처럼 끈적','잼처럼','strawberry','mixed berry','wild berry','jam','jammy']],
  ['cherry',      ['체리','크랜베리','다크체리','건체리','건크랜베리','오미자','오미자청','구기자','붉은 과일','붉은 과일류','적색 과일','과일 중심','과일 향이 두드러짐','과일의 밑바탕','과일 향미','과일 향','풍부한 과일향','짙은 과일향','복합적인 과일향','대단히 복합적인 과일향','cherry','cranberry','dark cherry','maraschino','morello','dried cherry','goji','red fruit','red fruits']],
  // tropical
  ['pineapple',   ['파인애플','트로피컬','잭프루트','pineapple','tropical','jackfruit']],
  ['mango',       ['망고','mango','alphonso','ataulfo']],
  ['melon',       ['멜론','메론','리치','구아바','용안','melon','lychee','guava','honeydew','longan']],
  ...
  ['passion',     ['패션후르츠','패션','패숀','히비스커스','열대과일','열대과일 뉘앙스','이색적','이색적이고 독특한 아로마','독특한 아로마','passion','hibiscus','passionfruit','passion fruit','tropical fruit','exotic']],
  ['olive',       ['올리브','올리브오일','올리브 오일','olive','olive oil','green olive']],
  ...
  // sweet
  ['caramel',     ['캐러멜','카라멜','다크캐러멜','다크카라멜','캐러멜소스','카라멜라떼','캐러멜라떼','토피','버터스카치','캐러멜화','캐러멜화된','캐러멜화된 설탕','caramel','dark caramel','toffee','butterscotch','dulce de leche','caramelized']],
  ['cotton',      ['솜사탕','솜사탕향','cotton candy','candy floss']],
  ['chocolate',   ['초콜릿','초콜렛','다크초코','카카오','밀크초콜릿','화이트초콜릿','모카','초콜릿향','초코퍼지','코코아 톤','풍부한 초콜릿','풍부한 초콜릿 향미','진한 초콜릿','다크 초콜릿','다크 코코아','쓴맛','강한 쓴맛','cacao','cocoa','chocolate','dark chocolate','milk chocolate','white chocolate','mocha','ganache','fudge','bitter']],
  ['honey',       ['꿀','벌꿀','꿀향','허니','honey향','꿀맛','대추','당밀','사탕수수 당밀','벌집','시럽','시럽 같이','시럽 같이 풍부','molasses','treacle','agave','honey','honeycomb','clean sweet','clean sweetness']],
  // 단맛 family — map to honey by default
  ['honey',       ['단맛','단 맛','달콤','달콤함','달큰','달달','맑은단맛','맑은 단맛','깨끗한단맛','깨끗한 단맛','깔끔한단맛','깔끔한 단맛','클린 스위트','깊은 단맛','뛰어난 단맛','강렬한 단맛','특출나고 뚜렷한 단맛','뚜렷한 단맛','우아한 단맛','부드러운 단맛','둥글고 부드러운 단맛','둥근 단맛','감미로움','달콤한 여운','잘 익은 과육의 달콤함','과육의 달콤함','달콤한 향','전반적으로 달콤한 향','sweet','sweetness','sweet note','sweet finish']],
  ...
  ['herb',        ['허브','민트','바질','타임','로즈마리','쑥','쑥향','판단','레몬그라스','허브향','허브 향','짭짤한 허브','짭짤한 허브 풍미','herb','mint','basil','thyme','rosemary','pandan','mugwort','sage','eucalyptus','lemongrass','herbal']],
  ['grass',       ['풀향','풀내','그라시','헤이','덜익은','새싹','신선한 풀향','짙은 풀향','신선한/짙은 풀향','건초','건초향','풋내','익지 않은 고구마','덜 익은 과일','완두콩 껍질','마른 나무','hay','grassy','green','vegetal','under-ripe','fresh grass','cut grass','straw','dry wood']],
  ['earthy',      ['흙','흙내','어시','머디','짙은 흙내음','흙내음','곰팡내','눅눅한 향','눅눅한','쩐내','오래된 쩐내','골판지','종이','구조감','구조감 있는 신맛','earthy','muddy','damp','musty','dirty','moldy','mold','cardboard','papery','old']],
  // roasty
  ['roasty',      ['로스티','비스킷','토스트','그레이엄','탄맛','roasty','biscuit','graham','toast','browned']],
  // nuts
  ['nut',         ['견과','견과류','아몬드','헤이즐넛','호두','밤','피칸','마카다미아','피스타치오','잣','프랄린','구운 헤이즐넛','견과류 뉘앙스','견과류 아로마','묵직한 견과류','짙은 견과류','헤이즐넛/호두','아몬드 등 견과류','nut','hazel','hazelnut','almond','walnut','chestnut','praline','pecan','macadamia','pistachio','marzipan','pine nut','toasted nut']],
  ...
  ['nutmeg',      ['넛메그','시나몬','계피','육두구','후추','흑후추','후추향','스파이시','향신료','카다멈','브라운 스파이스','자극적인 톡 쏘는 향','자극적인','톡 쏘는 향','nutmeg','cinnamon','allspice','pepper','black pepper','white pepper','spicy','spice','brown spice']],
  ['clove',       ['정향','카다몬','일라이치','팔각','올스파이스','clove','cardamom']],
  ['butter',      ['버터','크림','버터크림','밀키','매끄럽고 둥근 질감','매끄럽고 복합적인 질감','매끄러운 바디','매끄러운 질감','둥근 질감','풍성한 마우스필','마우스필','요거트/크림 같은 질감','요거트/크림','크림 같은 질감','흙/버터','흙/버터 같은 둥근 질감','부드러움','부드럽고','부드럽고 향긋함','일관되게 부드럽고 향긋함','가죽','가죽 향','butter','cream','clotted cream','browned butter','milky','brown butter','smooth','silky','round','mouthfeel','leather']],
  // tea & beverage
  ['tea',         ['홍차','얼그레이','보이차','녹차','우롱','루이보스','다르질링','차향','홍차 같은','홍차 같은 느낌','향신료 차','아놀드 파머','earl grey','green tea','oolong','pu-erh','rooibos','darjeeling','assam','ceylon','sencha','matcha','black tea','spiced tea','arnold palmer']],
  ['cereal',      ['그래놀라','오트밀','후르트링','시리얼','귀리','곡물/맥아','브라운 로스트','muesli','granola','oatmeal','porridge','oat','brown roast']],
  ['tobacco',     ['타바코','담배','재','어시향','탄향','탄내','담배잎','담배잎 향','담배 잎','파이프 담배','아린 맛','아린맛','tobacco','ashy','ash','smoky dark','tobacco leaf','pipe tobacco']],
  ['wood',        ['우디','시더','오크','나무','우드','나무향','마른 나무','woody','cedar','oak','sandalwood','wood','teak','dry wood']],
  ['wine',        ['와인','포트와인','레드와인','화이트와인','와이니','포트','와인 같은','와인 같은 산미','와인 같은 향미','적포도주','적포도주 같은 느낌','시럽 같이 풍부하고 묵직한 바디','wine','red wine','white wine','port','winey','vinous']],
  ['malt',        ['몰트','맥아','그레인','곡물','슈가케인','슈가 케인','식혜','누룽지','malt','grain','barley','rye','sugar cane','wheat','rice','sikhye']],
  ['mineral',     ['미네랄','감칠맛','짭조름','짠맛','짭짤','구연산','아세트산','뷰티르산','이소발레르산','사과산','부싯돌','약품','약품 냄새','mineral','umami','savory','salty','brine','minerality','terroir','citric acid','acetic acid','butyric acid','isovaleric','flint','medicinal']],
  ['smoke',       ['훈연','스모키','피트','훈제','스모키향','스모키 향','고무 타는','고무 타는 향','고무 타는 냄새','스컹크','스컹크 냄새','smoky','peaty','peat','burnt','campfire','smoked','rubber','skunk','burning rubber']],
  ['fermented',   ['발효','발효향','콤부차','막걸리','사케','미림','과발효','식초산','주류','시큼한','시큼한 향','시큼한 타르트','시큼한 타르트 같은 산미','과숙성','과숙성된 과일','펑키','펑키한 노트','짭조름하고 펑키한','짭조름하고 펑키한 노트','fermented','kombucha','winey-sour','lactic','sake','makgeolli','mirin','vinegar','acetic','boozy','sour','tart','funky','overripe']],
  ['cheese',      ['버터산','치즈향','젖산','유산균','치즈','butyric','cheesy','lactic acid','dairy sour']],
  ['champagne',   ['샴페인','샴페인산미','스파클링산미','스파클링','탄산산미','복합적인 산미','정제되고 세련된 산미','정제된 산미','세련된 산미','밝은 산미','밝고 쥬시한 산미','쥬시한 산미','쥬시','부드러운 산미','부드럽고 낮은 산미','낮은 산미','중상 등급의 산미','탄산수 같은 청량한 산미','청량한 산미','청량함','생동감','생동감 있는','가볍고 톡 쏘는 생동감','톡 쏘는','톡 쏘는 생동감','산미','신맛','구조감 있는 신맛','champagne','sparkling acidity','effervescent','acidity','bright acidity','vibrant','juicy','refined acidity']],
  ...
  ['yogurt',      ['요거트','요구르트','크림치즈','요거트 같은','요구르트 같은','yogurt','yoghurt','cream cheese','buttermilk','kefir']],
